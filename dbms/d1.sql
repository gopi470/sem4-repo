...
resv_id varchar2(4) constraint rid_fkk references reservation(resv_id),
amount decimal(10,2),
p_date DATE,
p_mode varchar2(10),
...
rem 1. Affordable_Boats


create or replace view Affordable_Boats as
select boat_id, boat_name, type, capacity,price from boat
...
rem  RESULT: Affordable_Boats is UPDATABLE



rem 2. Sailor_Reservation_View


create or replace view Sailor_Reservation_View as
select s.sailor_id,s.sailor_name,s.rating,r.resv_id,r.resv_date from sailor s 
join reservation r on s.sailor_id=r.sailor_id;

select * from Sailor_Reservation_View;

update Sailor_Reservation_View
set rating = 'A'
where sailor_id = 'S003';



update Sailor_Reservation_View
set resv_date = sysdate
where sailor_id = 'S003';

rem RESULT:
rem sailor_id, sailor_name, rating is NOT UPDATABLE
rem resv_date: UPDATABLE - It belongs to the key-preserved table RESERVATION
rem Overall: Sailor_Reservation_View is PARTIALLY or NOT UPDATABLE



Rem 3. Create a view named Boat_By_Month


create or replace view Boat_By_Month as
select boat_id, to_char(sail_date , 'mon-yyyy') as sailor_month, count(*) as reservation_count from reservation
group by boat_id, to_char(sail_date, 'mon-yyyy')
having count(*)>1;

select * from Boat_By_Month;

update Boat_By_Month
set reservation_count = 5
where boat_id = 'B002';

rem RESULT: Boat_By_Month is NOT UPDATABLE



Rem 4. Create a view to display tourists who have made family reservations


create or replace view family_reservation as 
select t.tourist_id, t.tourist_name, count(*) as family_reservation_count from tourist t
join reservation r on 
r.tourist_id=t.tourist_id
where no_of_children >= 1
group by t.tourist_id, t.tourist_name
having count(*) >(
    select avg(fam_count) from (
        select count(*) as fam_count from reservation
        where no_of_children >= 1
        group by reservation.tourist_id
    )
);

select * from family_reservation;

update family_reservation
set family_reservation_count = 10
where tourist_id = 'T001';

rem RESULT: family_reservation is NOT UPDATABLE




Rem 5. create  a new view named Small_Affordable_Boats


create or replace  view Small_Affordable_Boats as
...
rem RESULT: Small_Affordable_Boats is UPDATABLE


SELECT column_name, updatable
FROM user_updatable_columns
WHERE table_name = 'SMALL_AFFORDABLE_BOATS';
